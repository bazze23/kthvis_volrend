...
RayCasting1PassEmptySpaceSkipping::RayCasting1PassEmptySpaceSkipping()
...
void RayCasting1PassEmptySpaceSkipping::Clean() {
...
void RayCasting1PassEmptySpaceSkipping::ReloadShaders() {
	cp_shader_rendering->Reload();
	m_rdr_frame_to_screen.ClearShaders();
}

bool RayCasting1PassEmptySpaceSkipping::Init(int swidth, int sheight) {
	if (IsBuilt()) Clean();
	...
	CreateRenderingPass();
	gl::ExitOnGLError("RayCasting1PassEmptySpaceSkipping: Error on Preparing Models and Shaders");
...
bool RayCasting1PassEmptySpaceSkipping::Update(vis::Camera* camera) {
	...
	gl::Shader::Unbind();
	gl::ExitOnGLError("RayCasting1PassEmptySpaceSkipping: After Update.");
	return true;
}

void RayCasting1PassEmptySpaceSkipping::Redraw() {
...
void RayCasting1PassEmptySpaceSkipping::MultiSampleRedraw() {
	m_rdr_frame_to_screen.ClearTexture();
...
void RayCasting1PassEmptySpaceSkipping::DownScalingRedraw() {
	m_rdr_frame_to_screen.ClearTexture();
...
void RayCasting1PassEmptySpaceSkipping::UpScalingRedraw() {
	m_rdr_frame_to_screen.ClearTexture();
...
void RayCasting1PassEmptySpaceSkipping::SetImGuiComponents() {
	ImGui::Separator();
	ImGui::Text("Step Size: ");
	if (ImGui::DragFloat("###RayCasting1PassEmptySpaceSkippingUIIntegrationStepSize", &m_u_step_size, 0.01f, 0.01f, 100.0f, "%.2f")) {
...
void RayCasting1PassEmptySpaceSkipping::FillParameterSpace(ParameterSpace& pspace) {
	pspace.ClearParameterDimensions();
	pspace.AddParameterDimension(new ParameterRangeFloat("StepSize", &m_u_step_size, 0.2, 2.0, 0.1));
}

void RayCasting1PassEmptySpaceSkipping::CreateRenderingPass() {
	...
	cp_shader_rendering->AddShaderFile(CPPVOLREND_DIR "structured/rc1pess/ray_marching_1pess.comp");
...
void RayCasting1PassEmptySpaceSkipping::DestroyRenderingPass() {
...
void RayCasting1PassEmptySpaceSkipping::RecreateRenderingPass() {
	DestroyRenderingPass();
	CreateRenderingPass();
